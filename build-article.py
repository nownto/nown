...
def build():
    ...
    head = head.replace('</style>', ".math{ font-family:var(--mono); text-align:center; font-size:1rem; "
                        ...
                        ".fn{ position:relative; display:inline-block; line-height:0 }\n"
                        ".fn .fnr{ font:600 .64em var(--mono); color:var(--bronze,#9c7635); background:none; "
                        "border:none; cursor:pointer; padding:0 .12em; vertical-align:super }\n"
                        ".fn .fnr:hover,.fn .fnr:focus{ text-decoration:underline; outline:none }\n"
                        ".fn .fnpop{ display:none; position:absolute; left:0; top:1.5em; z-index:30; "
                        "width:min(20rem,74vw); font:400 .8rem/1.55 var(--serif); text-align:left; "
                        "letter-spacing:0; color:var(--ink); background:var(--paper-2,#f3efe6); "
                        "border:1px solid var(--rule,#d6cfc1); border-radius:.55rem; padding:.65rem .8rem; "
                        "box-shadow:0 8px 28px rgba(0,0,0,.20) }\n"
                        ".fn .fnr:focus + .fnpop,.fn:hover .fnpop{ display:block }\n</style>", 1)
    ...
    top = top.replace('nown-whitepaper', 'nown-article')
